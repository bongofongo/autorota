...
- **Multi-role shift requirements silently lost across iCloud sync**
  ...
  - Patched: yes — pending user verification
  - Fix (per `SYNC_MULTIROLE_FIX.md`): added migration `025_role_requirements_sync.sql` (sync-only `role_requirements_json` TEXT mirror on `shifts` + `shift_templates`, backfilled from child tables); registered it in `db/mod.rs::run_migrations`; `set_shift_role_requirements` / `set_template_role_requirements` now also write the JSON mirror and bump `last_modified`; added `role_requirements_json` to the `shifts`/`shift_templates` arms of `syncable_columns` (push + apply automatic); `apply_remote_record` re-materialises child rows from the synced JSON via `replace_role_requirements` (NOT `set_*`, to avoid flipping `sync_status` back to 0). No Swift/FFI changes (field-agnostic pipeline); added doc comment on whole-list LWW in `SyncConflictResolver`. 4 new Rust tests in `db_integration.rs` (push, apply round-trip, clear-to-wildcard, 025 backfill) — all green. ~~**Before production:** add `role_requirements_json` String field to `Shift` + `ShiftTemplate` CloudKit record types.~~ **Superseded 2026-07-14:** `SyncRecordMapper` now stores each row's whole JSON in a single `payload` String field per CloudKit record type (`toCKRecord`/`fromCKRecord`), so no per-column CloudKit schema change is ever required again — `role_requirements_json` rides inside `payload` like every other column. One-time only: ensure a `payload` field exists on each record type in the Development schema (auto-created on first sync) and promote to Production once. Wire-format change was free (no live data yet). See `SyncRecordMapperTests`.
...
- **Not a bug: perceived sim/device behavior differences were per-install state (diagnosis only, no code change)**
  - Date diagnosed: 2026-07-20
  - Report 1: "Try the Demo" is a top-level Settings row on iPhone 13 Pro sim but lives inside Help & Guidance on iPhone 17 Pro Max sim. Cause: intentional design — `hasEverCompletedDemo` (UserDefaults, `DemoModeController`) hides the Settings row (`SettingsView.swift`) after first demo completion and shows "Replay Demo" in Help (`HelpCenterView.swift`) instead. Each sim has its own UserDefaults; both ran iOS 26.2 with same-day builds.
  - Report 2: boot animation plays on 13 Pro sim but "never plays" on real 13 Pro. Cause: Reduce Motion enabled on the device (confirmed by user) — `LoadingScreenView` pins the pocket-watch timeline at t=0 and shows a static logo for 0.8s. Fresh sims default Reduce Motion off.
  - Patched: no — behavior is as designed. Lesson for future reports: rule out per-install state (UserDefaults flags, accessibility settings, onboarding/demo completion) before suspecting device model, iOS version, or stale builds.

- **AutorotaAppPerfTests runner bootstrap crash killed every `make swift-test-app-*` run (test plans never attached to scheme)**
  - Date fixed: 2026-07-20 (pending verification)
  - Where / what / repro: `AutorotaApp.xcscheme` had `shouldAutocreateTestPlan="YES"` and no `<TestPlans>` element, so plain `xcodebuild test` (all three `swift-test-app-*` targets) auto-built a plan containing BOTH testables — booting the XCUITest perf runner, which macOS killed before bootstrap ("Early unexpected exit, operation never finished bootstrapping … Test crashed with signal kill") because the unit-test invocation lacks the sandbox/signing overrides only `swift-perf-macos` sets. Same root cause also meant `-testPlan Perf` (Makefile perf targets + perf.yml) could not resolve: `Perf.xctestplan` sat unreferenced at `platforms/apple/Apps/`, one level above the project, with a `containerPath` that only resolves next to the `.xcodeproj`. This was the long-standing "perf-runner bootstrap crash on clean main" and the reason the ViewModel CI job was shelved 2026-07-17.
  - Patched: yes — pending user verification
  - Fix: moved `Perf.xctestplan` next to the project, added a default `AutorotaApp.xctestplan` containing only `AutorotaAppTests`, and wired both into the scheme via `<TestPlans>` with `shouldAutocreateTestPlan="NO"`. `xcodebuild -showTestPlans` now lists both; `make swift-test-app-macos` runs green with zero perf-runner involvement.
