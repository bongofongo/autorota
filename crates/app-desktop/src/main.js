...
let editShiftTemplateId = null;
let cleanupCurrentView = null;
let lastScheduleWarnings = [];
// ─── API wrappers ───────────────────────────────────────────
...
function getWeekCategory(weekStart) {
...
function renderSidebarEmployees() {
    ...
    el.innerHTML = employees
        .map((e) => `<div class="employee-item" data-id="${e.id}">${e.name}</div>`)
        .join("");
    el.querySelectorAll(".employee-item").forEach((item) => {
        item.addEventListener("click", () => {
            selectedEmployeeId = parseInt(item.dataset.id);
  ...
  `;
    ...
    document.querySelectorAll(".item-row").forEach((row) => {
        row.addEventListener("click", (e) => {
            ...
            if (target.closest(".kebab-wrap"))
                return;
            selectedEmployeeId = parseInt(row.dataset.id);
            currentView = "employee-detail";
            document.querySelectorAll(".nav-btn").forEach((b) => b.classList.remove("active"));
            renderView();
        });
    ...
    document.querySelectorAll(".edit-emp").forEach((btn) => {
        btn.addEventListener("click", (e) => {
            e.stopPropagation();
            editEmployeeId = parseInt(btn.dataset.id);
...
function renderCreateEmployeeView() {
  ...
  `;
    ...
    document.getElementById("create-emp-btn").addEventListener("click", async () => {
        ...
        const rolesStr = document.getElementById("emp-roles").value.trim();
        const startDate = document.getElementById("emp-start-date").value;
        const targetWeekly = parseFloat(document.getElementById("emp-target-weekly").value);
        const deviation = parseFloat(document.getElementById("emp-deviation").value);
        const daily = parseFloat(document.getElementById("emp-daily").value);
        const notes = document.getElementById("emp-notes").value.trim() || null;
        const bank = document.getElementById("emp-bank").value.trim() || null;
        if (!name || !rolesStr || !startDate)
            return;
...
async function renderEditEmployeeView() {
    ...
    const emp = await invoke("get_employee", { id: editEmployeeId });
    if (!emp) {
        currentView = "employees";
        renderEmployeesView();
        return;
...
function createAvailGridInteraction(gridEl) {
    const allCells = Array.from(gridEl.querySelectorAll(".avail-cell"));
    const NUM_DAYS = 7;
    ...
    function selectRect(col1, row1, col2, row2) {
        ...
        for (let r = minRow; r <= maxRow; r++) {
            for (let c = minCol; c <= maxCol; c++) {
                ...
                const shadows = [];
                if (r === minRow)
                    shadows.push(`inset 0 ${B}px 0 0 ${SEL_BORDER}`);
                if (r === maxRow)
                    shadows.push(`inset 0 -${B}px 0 0 ${SEL_BORDER}`);
                if (c === minCol)
                    shadows.push(`inset ${B}px 0 0 0 ${SEL_BORDER}`);
                if (c === maxCol)
                    shadows.push(`inset -${B}px 0 0 0 ${SEL_BORDER}`);
...
async function renderEmployeeDetailView() {
    ...
    const emp = await invoke("get_employee", { id: selectedEmployeeId });
    if (!emp) {
        currentView = "employees";
        renderEmployeesView();
        return;
  ...
  `;
    ...
    // ─── Wire up both grids ────────────────────────────────
    const defaultGridEl = document.querySelector("#default-avail-wrap .avail-grid");
    const weekGridEl = document.querySelector("#week-avail-wrap .avail-grid");
    ...
    document.getElementById("save-week-avail").addEventListener("click", async () => {
        const btn = document.getElementById("save-week-avail");
    ...
    document.getElementById("reset-week-avail").addEventListener("click", async () => {
        ...
        const weekCells = weekGridEl.querySelectorAll(".avail-cell");
    ...
    function handleAvailKeydown(e) {
        const key = e.key.toLowerCase();
        if (key === "y") {
            activeGrid.applyState("Yes");
            e.preventDefault();
            if (activeGrid === defaultGrid)
                autoSaveDefault();
        }
        else if (key === "m") {
            activeGrid.applyState("Maybe");
            e.preventDefault();
            if (activeGrid === defaultGrid)
                autoSaveDefault();
        }
        else if (key === "n") {
            activeGrid.applyState("No");
            e.preventDefault();
            if (activeGrid === defaultGrid)
                autoSaveDefault();
        }
    ...
    function handleOutsideClick(e) {
        if (!defaultGridEl.contains(e.target) && !weekGridEl.contains(e.target)) {
...
function weekdaysToLetters(days) {
...
async function renderEditShiftTemplateView() {
  ...
  `;
    ...
    document.getElementById("save-tmpl-btn").addEventListener("click", async () => {
        ...
        const checked = Array.from(document.querySelectorAll(".day-checkboxes input:checked"));
        const selectedDays = checked.map((cb) => cb.value);
        const startVal = document.getElementById("tmpl-start").value;
        const endVal = document.getElementById("tmpl-end").value;
        const role = document.getElementById("tmpl-role").value.trim();
        const minEmp = parseInt(document.getElementById("tmpl-min").value);
        const maxEmp = parseInt(document.getElementById("tmpl-max").value);
        if (!name || !role || selectedDays.length === 0)
            return;
...
function renderShiftsView() {
  ...
  `;
    ...
    document.querySelectorAll(".item-row").forEach((row) => {
        row.addEventListener("click", (e) => {
            if (e.target.closest(".kebab-wrap"))
                return;
            editShiftTemplateId = parseInt(row.dataset.id);
    ...
    document.querySelectorAll(".edit-tmpl").forEach((btn) => {
        btn.addEventListener("click", (e) => {
            e.stopPropagation();
            editShiftTemplateId = parseInt(btn.dataset.id);
            currentView = "edit-shift-template";
            renderView();
        });
    });
  ...
  `;
    ...
    if (generateBtn) {
        generateBtn.addEventListener("click", async () => {
            ...
            try {
                const result = await invoke("run_schedule", { weekStart: selectedWeek });
...
function renderRotaGrid(weekdays, schedule) {
...
function updateWeekLabel() {
    ...
    if (el)
        el.textContent = formatWeekLabel(selectedWeek);
}
