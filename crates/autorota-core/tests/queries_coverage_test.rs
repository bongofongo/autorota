...
#[tokio::test]
async fn unstage_day_only_unstages_that_date() {
    let pool = test_pool().await;
    let rota_id = queries::insert_rota(&pool, week_start()).await.unwrap();

    // Create shifts on Mon(23) and Tue(24)
    let s1 = ShiftBuilder::new()
        .rota(rota_id)
        .no_template()
        .date(date(23))
    ...
    // Stage both (today = date(30) so both are in the past)
    let today = date(30);
    queries::stage_shifts(&pool, &[sid1, sid2], today)
        .await
        .unwrap();
    ...
    // Unstage only Mon(23)
    queries::unstage_day(&pool, rota_id, date(23))
        .await
        .unwrap();
    ...
    assert_eq!(staged[0], sid2);
}

#[tokio::test]
async fn list_past_shift_ids_returns_only_past() {
    let pool = test_pool().await;
    let rota_id = queries::insert_rota(&pool, week_start()).await.unwrap();

    // Past shift (Mon 23) and future shift (Wed 25)
    let past = ShiftBuilder::new()
        .rota(rota_id)
        .no_template()
        .date(date(23))
        .times(7, 12)
    ...
    let future = ShiftBuilder::new()
        ...
        .role("barista")
        .build();
    let past_id = queries::insert_shift(&pool, &past).await.unwrap();
    let _future_id = queries::insert_shift(&pool, &future).await.unwrap();

    // "today" is Tue 24 — only Mon 23 is past
    let past_ids = queries::list_past_shift_ids(&pool, rota_id, date(24))
        .await
        .unwrap();
    assert_eq!(past_ids, vec![past_id]);
}
